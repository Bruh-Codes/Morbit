...
import { getTransactionAction, getTransactionId } from 'modules/history/helpers';
import {
  actionFilterMap,
  ActionName,
  CowSwapSubset,
...
} from 'utils/swapAdapterHistory';
import { useShallow } from 'zustand/shallow';

import { useAppDataContext } from './app-data-provider/useAppDataProvider';

const COW_ENV = 'prod';
const APP_CODE_VALUES: string[] = [];
const APP_CODE_PER_SWAP_TYPE = {};
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const isChainIdSupportedByCoWProtocol = (_chainId: number): boolean => false;
...
export const useTransactionHistory = ({ isFilterActive }: { isFilterActive: boolean }) => {
  ...
  useEffect(() => {
    ...
    const newTransactions = sdkData.items.filter((transaction: any) => {
  ...
  const fetchCowSwapsHistory = async (first: number, skip: number) => {
    ...
    if (isChainIdSupportedByCoWProtocol(chainId)) {
      ...
      ).then((txns) => txns.filter((txn: any) => txn !== null));
    }
    ...
    const localCowTxns: TransactionHistoryItemUnion[] = await Promise.all(
      (localEntries as CowAdapterEntry[])
        .filter((e: any) => e.protocol === 'cow' && !apiCowIds.has(e.orderId))
    ...
    const localParaswapTxns: TransactionHistoryItemUnion[] = (
      ...
      .filter((e: any) => e.protocol === 'paraswap')
