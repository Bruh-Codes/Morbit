...
import { customAssetDomains, getCachedPermitDomain } from 'ui-config/permitConfig';
...
export interface PoolSlice {
  ...
  generatePermitPayloadForMigrationSupplyAsset: (..._args: any[]) => any;
  signStakingApproval: (..._args: any[]) => any;
}
...
> = (_, get) => {
  ...
  return {
    ...
    claimRewards: async ({ formattedReserves, selectedReward }) => {
      ...
      formattedReserves.forEach((reserve: any) => {
    ...
    generatePermitPayloadForMigrationSupplyAsset: () => undefined,
    signStakingApproval: () => Promise.resolve('0x'),
  };
};
